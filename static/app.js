...
function renderDashboardTable(data) {
    ...
    tbody.innerHTML = data.map(a => `
        <tr>
            ...
            <td>${Number(a.velocity_km_s).toFixed(2)}</td>
            <td>${Number(a.distance_km).toLocaleString()}</td>
            <td>${Number(a.max_diameter_km).toFixed(4)}</td>
        </tr>
    `).join('');
}
    ...
    try {
        ...
        container.innerHTML = `
            ...
            <div class="apod-info">
                ...
                <span class="date">${escapeHtml(apod.date)} ${apod.copyright ? '• © ' + escapeHtml(apod.copyright) : ''}</span>
...
function renderAsteroidsTable(data) {
    ...
    tbody.innerHTML = data.map(a => `
        <tr>
            ...
            <td>${Number(a.distance_km).toLocaleString()}</td>
            <td>${Number(a.max_diameter_km).toFixed(4)}</td>
            <td>${Number(a.min_diameter_km).toFixed(4)}</td>
        </tr>
    `).join('');
}
