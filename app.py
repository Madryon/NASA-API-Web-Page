...
def get_asteroids():
    ...
    try:
        ...
        return jsonify({"success": False, "error": "Dates must use YYYY-MM-DD format"}), 400

    if end_obj < start_obj:
        return jsonify({"success": False, "error": "end_date must be on or after start_date"}), 400
    if (end_obj - start_obj).days > 7:
        return jsonify({"success": False, "error": "Date range cannot exceed 7 days"}), 400
